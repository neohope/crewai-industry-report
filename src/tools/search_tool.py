...
_skill_path_cache: Optional[Path] = None


def _find_skill_path() -> Optional[Path]:
    """查找byted-web-search技能路径"""
    global _skill_path_cache
    if _skill_path_cache is not None:
        return _skill_path_cache
    current_dir = Path(__file__).parent.parent.parent
    skill_dir = current_dir / "skills" / "byted-web-search"
    if skill_dir.exists():
        _skill_path_cache = skill_dir
        return skill_dir
    return None


def _search_with_skill(query: str, max_results: int = 10,
                      time_range: str = None,
                      auth_level: int = 0,
                      query_rewrite: bool = False) -> List[Dict[str, Any]]:
    """使用byted-web-search技能进行搜索"""
    skill_path = _find_skill_path()
    if not skill_path:
        return [{"error": "byted-web-search技能不可用", "info": "请确认skills/byted-web-search目录存在"}]

    script_path = skill_path / "scripts" / "web_search.py"
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            return _parse_skill_output(result.stdout)
        else:
...
def _parse_skill_output(stdout: str) -> List[Dict[str, Any]]:
...
def web_search(query: str, max_results: int = 10,
    ...
    results = _search_with_skill(query, max_results, time_range, auth_level, query_rewrite)
...
def news_search(query: str, max_results: int = 10) -> str:
    ...
    results = _search_with_skill(query, max_results)
...
class WebSearchTool:
    ...
    def __call__(self, *args, **kwargs) -> str:
        return web_search.func(*args, **kwargs)
...
class NewsSearchTool:
    ...
    def __call__(self, *args, **kwargs) -> str:
        return news_search.func(*args, **kwargs)
